...
def calculate_narrator_votes(votes_df):
    ...
    for rnd in rounds:
        ...
        total_players = round_data['Player'].nunique() - 1  # Exclude narrator
        
        # Identify votes for the narrator's card
...
def compute_combined_averages(file_list, sheet_name, value_cols):
    ...
    # Loop through each file and read the specific sheet
    for file_path in file_list:
        df = pd.read_excel(file_path, sheet_name=sheet_name)
        player_type_col = df.columns[0]  # Assume the first column contains player names
        combined_data = pd.concat([combined_data, df], ignore_index=True)

    # Filter data by player type (GPT or Bot)
    ...
    # Calculate averages for each value column
...
def summarize(path_to_save, file_list):
    with pd.ExcelWriter(os.path.join(path_to_save, "summarize.xlsx")) as writer:
        # Compute and write averages for each relevant sheet
